...
app.post('/api/users', async (req, res) => {
  ...
  if (!name || !cls) {
    return sendError(res, 400, 'name和class为必传字段');
  }
  const data = await readData();
...
// 12. 摄像头画面 — GET：有上传则返回最新图片，否则生成占位图
app.get('/api/camera', async (req, res) => {
  ...
  try {
    ...
    <svg width="480" height="320" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect x="10" y="10" width="30" height="4" fill="#00ff88" opacity="0.8"/>
      <rect x="10" y="10" width="4" height="30" fill="#00ff88" opacity="0.8"/>
      <rect x="440" y="10" width="30" height="4" fill="#00ff88" opacity="0.8"/>
      <rect x="466" y="10" width="4" height="30" fill="#00ff88" opacity="0.8"/>
      <rect x="10" y="286" width="30" height="4" fill="#00ff88" opacity="0.8"/>
      <rect x="10" y="280" width="4" height="30" fill="#00ff88" opacity="0.8"/>
      <rect x="440" y="286" width="30" height="4" fill="#00ff88" opacity="0.8"/>
      <rect x="466" y="280" width="4" height="30" fill="#00ff88" opacity="0.8"/>
    </svg>`;
...
// 12a. ESP32 上传图片（multipart/form-data，字段名 image）
app.post('/api/camera', upload.single('image'), async (req, res) => {
  try {
...
// 12b. ESP32 上传图片（base64 JSON，字段: { image: "base64字符串" }）
app.post('/api/camera/base64', async (req, res) => {
  try {
